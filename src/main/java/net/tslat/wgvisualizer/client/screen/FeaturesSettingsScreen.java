...
public class FeaturesSettingsScreen extends Screen {
	...
	protected void init() {
		...
		addButton(rootWidget = new JsonObjectsField(guiRootX, guiRootY + 20, I18n.format(TITLE.getKey()), null, currentSettings, editedSettings, new StringTextComponent(""), this::addButton).setSaveFunction(FeaturesSettingsScreen::saveChanges));
	}
	...
	private static JsonObject getCurrentFeaturesJson() {
		...
		return ClientOperations.currentWorldgenData.get(Operations.GenCategory.FEATURES.toString()).getAsJsonObject();
	}
}
