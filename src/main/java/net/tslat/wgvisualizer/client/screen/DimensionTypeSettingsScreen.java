...
public class DimensionTypeSettingsScreen extends Screen {
	...
	protected void init() {
		...
		addButton(rootWidget = new JsonObjectsField(guiRootX, guiRootY + 20, I18n.format(TITLE.getKey()), null, currentSettings, editedSettings, new StringTextComponent(""), this::addButton).setSaveFunction(DimensionTypeSettingsScreen::saveChanges));
	}
	...
	private static JsonObject getCurrentDimensionTypeJson() {
		...
		return ClientOperations.currentWorldgenData.get(Operations.GenCategory.DIMENSION_TYPE.toString()).getAsJsonObject();
	}
}
