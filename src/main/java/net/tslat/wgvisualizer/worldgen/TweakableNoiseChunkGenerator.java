...
import net.tslat.wgvisualizer.Registries;
...
public class TweakableNoiseChunkGenerator extends ChunkGenerator {
	public static final Codec<TweakableNoiseChunkGenerator> CODEC = RegistryLookupCodec.getLookUpCodec(Registry.BIOME_KEY).xmap(registry -> new TweakableNoiseChunkGenerator(registry, 1L, false), TweakableNoiseChunkGenerator::getBiomeRegistry).stable().codec();
	...
	public TweakableNoiseChunkGenerator(Registry<Biome> biomeRegistry, long seed, boolean isPreServer) {
		this(biomeRegistry, new DimensionStructuresSettings(Optional.ofNullable(null), new HashMap<Structure<?>, StructureSeparationSettings>()), seed, isPreServer);
	}

	public TweakableNoiseChunkGenerator(Registry<Biome> biomeRegistry, DimensionStructuresSettings structureSettings, long seed, boolean isPreServer) {
		super(getBiomeProvider(isPreServer, biomeRegistry, seed), getBiomeProvider(isPreServer, biomeRegistry, seed), structureSettings, seed);

		this.biomeRegistry = biomeRegistry;
		this.field_236084_w_ = seed;
		DimensionSettings dimensionSettings = DimensionSettings.func_242743_a(structureSettings, false, new ResourceLocation(WorldGenVisualizer.MOD_ID, "tweakable"));
	...
	private static BiomeProvider getBiomeProvider(boolean beforeServer, Registry<Biome> biomeRegistry, long seed) {
		return beforeServer ? new OverworldBiomeProvider(seed, false, false, biomeRegistry) : new SingleBiomeProvider(Registries.getTweakableBiome(biomeRegistry));
	}
